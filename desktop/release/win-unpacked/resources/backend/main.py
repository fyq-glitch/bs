...
session=ort.InferenceSession(r"C:\Users\fyq\Desktop\bs\model\runs\detect\n12\weights\best.onnx",providers=["CPUExecutionProvider","AzureExecutionProvider"])
...
def draw_detections(image: Image.Image, detections: list) -> Image.Image:
    ...
    return image
@app.post("/detect/")
async def detect_items(file: UploadFile = File(...),confidence_threshold:float=Form(0.5),iou_threshold:float=Form(0.5)):
    if not file:
