...
class DistillationLoss(nn.Module):
    ...
    def __call__(self, preds, batch):
        ...
        student_preds_raw, student_feats = preds[0][0], preds[0][1] if isinstance(preds, tuple) else preds
        ...
        with torch.no_grad():
            ...
            teacher_pred_distri, teacher_pred_scores = torch.cat(
                [xi.view(teacher_feats[0][0].shape[0], self.no, -1) for xi in teacher_feats[0]], 2
            ).split((self.reg_max * 4, self.nc), 1)
            teacher_neck_features_for_distill=teacher_feats[1]
        ...
        if self.kdf_weight > 0 and self.teacher_model and student_neck_features and teacher_neck_features_for_distill:
            kdf_loss_val = self._distill_feature_loss(student_neck_features, teacher_neck_features_for_distill)
